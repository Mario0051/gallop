...
static void Decode85(const unsigned char* src, unsigned char* dst)
{
	while (*src) {
		unsigned int tmp =
			Decode85Byte(src[0]) + 85 * (Decode85Byte(src[1]) + 85 * (Decode85Byte(src[2]) + 85 * (Decode85Byte(src[3]) + 85 * Decode85Byte(src[4]))));
		dst[0] = ((tmp >> 0) & 0xFF);
		dst[1] = ((tmp >> 8) & 0xFF);
		dst[2] = ((tmp >> 16) & 0xFF);
...
{
	const unsigned int buf_decompressed_size = stb_decompress_length((const unsigned char*)data_size);
	unsigned char* buf_decompressed_data = (unsigned char*)IM_ALLOC(buf_decompressed_size);
	stb_decompress(buf_decompressed_data, (const unsigned char*)data, (unsigned int)data_size);

	return LoadTextureFromMemory((const void*)buf_decompressed_data, buf_decompressed_size, out_texture);
}

bool LoadTextureFromMemoryCompressedBase85(const char* data, GallopImage& out_texture)
{
	int compressed_size = (((int)strlen(data) + 4) / 5) * 4;
	void* compressed_data = IM_ALLOC((size_t)compressed_size);
	Decode85((const unsigned char*)data, (unsigned char*)compressed_data);

	return LoadTextureFromMemoryCompressed((const void*)compressed_data, compressed_size, out_texture);
}
} // namespace gui
} // namespace gallop
...
static unsigned int stb_decompress_length(const unsigned char* input) { return (input[8] << 24) + (input[9] << 16) + (input[10] << 8) + input[11]; }
